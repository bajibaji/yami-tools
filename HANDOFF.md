...
| 试玩窗口的 `Scene.binding`（第 50 项顺带发现） | 待查证 | 编辑器侧已确认引擎没有 `Scene.binding`（改用 `Scene.meta`）；但试玩窗口是另一份运行时，`getSceneDetails`（`probe-core.js` 约 1253 行）与「秒杀全图怪」（约 5385 行）仍按 `Scene.binding` 取场景 —— 本机没有那份运行时代码可读，真机试玩时看一眼面板里「场景实体」有没有数据即可判定 |
