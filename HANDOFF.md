...
| 当前版本 | `v1.11.5`（单一事实源：`manifest.json` 的 `version`） |
...
1. **真机 GUI 走查**：**已用 MCP（浏览器 MCP 连编辑器 9222）走掉大半** —— 打 `@` 候选与高亮 ✓、回答下的「复制」（toast + 剪贴板实测）✓、「重新生成」（并借此抓到 v1.11.3 那个会丢对话的轮次错位）✓。**还剩两条**：
   ① **审批续跑顺序**（停在确认 → 勾授权 → 执行修改 → 看新的行是否落在同一回合组里）—— 能自动化，但要**真的让 AI 改一个工程文件**，需用户点头；
   ② **在场感知口径**（高亮选中技能 → 鼠标移画布 → 顶栏应仍说"选中「xxx」"）—— 要画布/资源树的鼠标操作，**助手做不到**，只能用户点。
   走查前先 `node build.cjs --deploy`；插件代码改动后**刷页面即可加载**（`navigate_page` reload），不必重启编辑器。
2. 用一个真实需求（例如改主菜单某处 UI）走完整链路：搜索 → 精读（`key`）→ 预览 → 确认 → 编译 → 变更小结 → 撤销，检验端到端手感。
3. 把 3.3 里那 13 条「待用户确认」逐条收口（宿主侧都有真链路验收，只差真机看一眼）；**收完再谈加新能力** —— 没有验收的功能等于没做，功能越多欠账越大。
