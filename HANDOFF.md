...
1. **真机 GUI 走查**：已用 CDP 连编辑器 9222 走掉大半 —— 打 `@` 候选与高亮 ✓、回答下的「复制」✓、「重新生成」✓（v1.11.3 那个会丢对话的轮次错位就是这条抓到的）、点主页卡片开面板 ✓、试玩冒烟 ✓、场景实体 ✓、在场感知口径 ✓（并抓出「容器当控件名」的缺陷）。**还剩两条，都只能由用户做**：
   ① **审批续跑顺序**（停在确认 → 勾授权 → 执行修改 → 看新的行是否落在同一回合组里）—— 面板 DOM 那一环能自动化，但前提是**真的让 AI 改一个工程文件**，需用户点头；
...
3. 3.3 里那 13 条「待用户确认」的收口状态（2026-09-24 结清）：**9 条已由助手真机收口**（打 `@` 候选与高亮、回答下的「复制」「重新生成」、点主页卡片开面板、试玩冒烟、场景实体、在场感知口径、界面细节四项、对话导出、审批续跑顺序），**3 个真机缺陷当场修掉**（v1.11.6）。**只剩 1 条**：编辑器热更新失败时的真机表现 —— 要制造"引擎未暴露 `YamiEngine`"的条件才复现得了，成本高收益低，**按已知限制留着**（正常热更新路径已被整包快照测试覆盖）。**收完再谈加新能力** —— 没有验收的功能等于没做。
