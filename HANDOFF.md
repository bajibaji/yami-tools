...
- **多维能力复合体演进蓝图**：
  ...
  9. `[v0.8.0 规划]` **事件指令级时间线**：最近 20 步事件流水（启动/执行/挂起/等待，指令名经 `Data.events` 原始 commands + index 反查——编译产物为匿名闭包需反查）+ 幽灵事件侦探（挂起时长标注、宿主已销毁标红、一键终止 `finish()`）；
  10. `[v0.9.0 规划]` **存档台改动 Diff 草稿**：未保存修改列表（旧值→新值）+ 一键还原 + 载入上一次备份；
  11. `[v0.9.0 规划]` **本地化漏译轻量提示**：en 语言下检测 UI 文本空串/GUID 原样出现 → 控制台页温和汇总；
