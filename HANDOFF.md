...
3. 3.3 里那 13 条「待用户确认」的收口状态（2026-09-24 结清，**全清零**）：**9 条已由助手真机收口**（打 `@` 候选与高亮、回答下的「复制」「重新生成」、点主页卡片开面板、试玩冒烟、场景实体、在场感知口径、界面细节四项、对话导出、审批续跑顺序），**3 个真机缺陷当场修掉**（v1.11.6），**最后 1 条**（编辑器热更新失败时的真机表现）经用户 2026-09-24 明确「可以无视了」→ **按已知限制归档，不再跟进**（正常热更新路径已被整包快照测试覆盖）。**收完再谈加新能力** —— 没有验收的功能等于没做。
