...
> **文档定位**：记录本套件的**系统架构剖析、底层工作原理、时间线演进历史、高价值核心经验与防踩坑档案**，作为跨开发者与 AI 协同的唯一技术基线与记忆中枢（SSOT）。当前版本：`v0.4.0`。
...
- **2026-09-03 上午 · 双模诊断体系与原型级排查落地 (v0.4.0)**
...
- **2026-09-03 10:30 · 稳定性与体验终极加固 (v0.4.0)**
...
- **2026-09-03 14:10 · 契约化解耦与零依赖构建自检体系落地 (v0.4.0 当前版本)**
...
- **现象**：明明版本没变，一启动试玩却总是弹出写死的“发现新版本 v0.4.0”升级条，更新后下次启动依然在。
...
### 2026-09-04 [里程碑] 控制台报错 (Error Debugger) 5大核心维度全面落地
- **1. 引擎专属白话诊断库扩充**：涵盖空指针目标属性解引用、公共事件死循环/爆栈死锁、场景地形与 Autotile 加载越界、插件自定义指令参数异常、WebGL 图形管线纹理/Shader 错误、音频解码播放阻断、NaN 无效数值计算。
- **2. 同类高频错误指纹聚合**：引入 fingerprint 错误指纹算法，针对帧循环中持续抛出的同源异常智能累加频次（[xN次]），标注首末发生时间戳，杜绝异常列表被无谓刷屏。
- **3. 源码上下文就地提取与定位直达**：自动解析报错物理工程路径，提取报错行上下 7 行源码片段，高亮标记出错行；提供【定位文件】按钮，一键调起操作系统资源管理器直达出问题的文件。
- **4. 分类过滤与实时搜索**：顶部提供 [全部]、[高频]、[空指针]、[方法丢失]、[插件指令]、[场景地形]、[资源404]、[控制台] 标签式过滤器，支持按关键字实时过滤搜索。
- **5. 一键导出 Markdown 结构化诊断报告**：一键生成包含硬件环境、游戏状态、场景、帧率以及全部异常详细调用栈与源码片段的专业报告，自动复制到剪贴板并可落盘保存为 .md 文件。
- **6. 测试验证**：编写并通过 24 项全维度自动化测试（e2e-error-debugger-test.cjs 100% 全绿完全通过）。
