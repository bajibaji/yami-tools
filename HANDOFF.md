...
| 当前版本 | `v1.11.2`（单一事实源：`manifest.json` 的 `version`） |
...
1. **真机 GUI 走查（唯一没做过的验收方式，也是当前最大的欠账）**：挑最容易坏的三条一次走完 ——
   ① **审批续跑顺序**（停在确认 → 勾授权 → 执行修改 → 看新的行是否落在同一个回合组里、卡片是否从「运行中」收尾）；
   ② **`@` 引用手感**（打 `@` 出候选 → `@启动` 第一条是「启动游戏事件」→ `Enter` 插入 → `Esc` 只关浮层）；
   ③ **在场感知口径**（高亮选中某技能后把鼠标移到画布/检视器空白处，顶栏应仍说"选中「xxx」"；停在有名字的控件上才说"停在「攻击力」"）。
   走查前先 `node build.cjs --deploy`，确保生产镜像是当前源。
