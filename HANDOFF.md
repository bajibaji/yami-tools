...
| 当前版本 | `v1.11.6`（单一事实源：`manifest.json` 的 `version`） |
...
| ⚠️ 启动工作目录必须＝编辑器安装目录 | `main.ts:330-335`（2026-09-24 实测） | 引擎那句是 `const extensionPath = path.join('./extension')` —— **相对路径，相对进程的工作目录**。若编辑器不是从安装目录启动（命令行/脚本/自定义快捷方式/计划任务等 cwd 不对），它会解析到别处：`fs.existsSync` 为假 → **`fs.mkdirSync` 在那边建一个空 extension 目录** → 遍历空目录 → **零扩展加载**。现象与铁律㊵那次事故（v1.0.0→v1.2.0 插件凭空消失）**一模一样，但根因完全不同**，极易误判成"更新把插件搞坏了"。实测对照：同一个 exe，cwd=母仓库启动 → 页面里 0 个 `yami*` 元素、插件面板全无；cwd=安装目录启动 → 319 个元素、HUD 与 AI 面板齐备。**排查「插件不见了」第一步：先看是不是启动目录不对**（顺带检查有没有被误建的空 `extension/` 目录）。 |
...
- **多维能力复合体演进蓝图**：
  ...
  10. `[已落地 v1.0.0]` **AI 全能副驾（AI Agent & MCP Hub）**：内置 `ai-agent.js` 与 `ai-host.js`，深度无缝桥接内置 `runtime/yami-mcp`（35 类工具）。白话驱动 Open Yami 引擎与工程全流程控制，包含写代码/编译、事件/数据表读写、5967 编辑器动作桥、5966 试玩交互桥、5968 本地代理桥、DPAPI 凭证加密存储、大文件截断与删除引用保护、三步确认审批与原子安全回滚、**输入框打 `@` 选工程文件**（第 16 轮：候选由宿主 `/files` 从 MCP 取，排除图片/音频并按工程缓存 60 秒；插入的只是一段文本，不改协议）；
...
- MCP 侧：`ai-host` 以 stdio 拉起 `runtime/yami-mcp/server.js`（JSON-RPC 2.0），工具数以 `tools/list` 为准（原始表 41 项；内置模型可见 40 项——`cdp_eval` 由宿主侧 `HIDDEN_TOOLS` 过滤，只留给外部 MCP 客户端与路线 B）。
- **上表三个端口是「默认值」，不是保证值**（v1.10.8 / v1.10.9）：5968 / 5967 / 5966 被别的程序占用时都往后试（最多 20 个），成功后把**真实端口**写进 `<配置目录>/agent-port` 与 `<kind>-port`；读端统一走 `runtime/yami-mcp/modules/bridge-port.js`（环境变量 > 端口文件 > 默认），`runtime-bridge` 每次请求重新解析（试玩可能换端口起来）。**排查「连不上」时先读端口文件，别假定端口号** —— 这条机制的起因就是 Steam 占了 5968 导致助手起不来，而当时的报错话术把用户引向了「模型接口」这个错方向（见 3.2 第 59 项）。
...
| 内置 MCP 工具集（41 项，模型可见 40 项：读 / 写 / 搜 / 编译 / 事件编排 / 数据表 / 备份 / 试玩冒烟 / 编辑器上下文…） | 已落地 | `runtime/yami-mcp/` |
| 代理能力（子任务委派给子代理） | 未做（P2） | 见 3.3 |
| 计划模式（Plan Mode） | 明确不做 | 用户拍板不需要 |

## 3.2 最近一轮完成（2026-09-11 → 09-24）
...
57. **补齐 4 项引擎级能力（v1.10.7，2026-09-17）**：
    - **① RLE 真编解码**：新增 `runtime/yami-mcp/modules/rle.js`，照引擎 `codec.ts` 逐行移植 `encodeTiles` / `encodeTerrains`（含 `encodeClone`）。实测本机工程 7 个场景 `terrains` + 12 张瓦片地图 `code`，**解码后重编码与磁盘串逐字节一致**；`write_resource` 的判据从「不许写短」升级为「必须解得开」（第 47 项那条安全网的升级版，尺寸缺失时退回老判据）。
    - **② 两个只读工具**：`list_attributes`（属性表 id/key/name/type，支持模糊查与按 id 反查）与 `read_tilemap`（解压地图给模型改）—— 属性键是 id，本机 `a5fd5e9f229abb2d`=生命值、`a8451228fe0c120a`=最大生命值。工具表随之从 37 项增至 **41 项**（模型可见 40 项）。
    - **③ 插件 lint**：`validate_project` 新增三类真问题检查（全局插件类名冲突 / `onBeforeSave` 没用 `define()` / 对只读 `data.plugins` 赋值）。
    - **④ 变量引用体检**：引用不存在的全局变量、类型与初始值不符（引擎两种都**静默丢弃**，不报错）。真机实测本机工程查出 22 处 `setNumber` 引用了不存在的全局变量。
    - **新增套件** `tests/test-mcp-data-rules.cjs`（14 项）锁住这四项；README 套件数 34→35。

58. **插件 lint 口径收窄 + 一条假红的真因（2026-09-17）**：
    - 类名冲突只对**已启用**的全局插件报警：停用的插件引擎在 `ScriptManager.create` 阶段就过滤掉、根本不会被实例化，老口径会报一条「看着吓人其实没事」的冲突（本机那对 DialogueSystem 正是一个启用一个停用）。过滤后只剩 1 条真冲突。
    - `test-ai-agent` 的「上游 500 要如实失败」**不是宿主 bug**，是测试的时序假设过期 —— 宿主现在要先备齐 MCP 工具表与环境快照才发第一次模型请求，测试死等的 200ms 不够，假模型没看到「慢一点坏」就走了成功分支。改法：假模型收到该请求时发信号，测试等信号（3s 兜底）再推引导。**教训**：给假模型加一行「最后一条 user 消息」日志，真相立刻出来 —— 假红先查测试的时序，别急着改产品。

59. **AI Host 端口自适应（v1.10.8，2026-09-19，用户报「拉取模型列表失败：AI 助手启动超时」）**：
    - **根因**：端口 **5968 被 Steam 占着**（实测该端口的监听进程是 `D:\Steam\steam.exe`），助手起不来，面板等 4.5 秒就报超时；那句"有些本地服务不提供 /models 接口"是固定附带的提示，跟这次无关 —— 报错话术把用户往错方向引。
    - **改法**（遵循「能在后端解决的绝不弹给用户设置」）：启动先试 5968，被占就往后试（最多 20 个），成功后把真实端口写进 `<配置目录>/agent-port`；面板启动前清掉旧端口文件、轮询时每步刷新端口，之后所有请求都打真实端口。端口上"是自己人"的竞态仍按老行为认领失败退出（前端复用已有实例）。
    - **新增** `tests/test-ai-host-port.cjs`（5 项）：故意占住端口 → 断言换到下一个端口并写文件、新端口 `/status` 能应答、第二个实例识趣退出、原实例仍活着。

60. **双桥端口自适应 + 模型名兜底（v1.10.9，2026-09-19）**：
    - 与 5968 同类的另两个坑一并堵上：5966（试玩推流）/ 5967（编辑器动作桥）改成共用的 `bindBridge`，被别的程序占用就往后试（最多 20 个），成功把**真实端口**写进插件数据目录 `<kind>-port`；新增 `runtime/yami-mcp/modules/bridge-port.js` 作为端口唯一真源（环境变量 > 端口文件 > 默认），`runtime-bridge` 每次请求重新解析（试玩可能换端口起来），`editor-bridge` 与 `ai-host` 读同一份文件。
    - **修前是假报错**：端口被占时试玩桥只打一行 warn，AI 那边却报「游戏没在试玩」。
    - 面板拉不到模型列表时，自动把默认模型名 `deepseek-chat` 填进输入框并如实说明，用户可直接开聊。

61. **按引擎源码逐条对齐：13 组修复（v1.10.11，2026-09-21）**：对照引擎源码（`Project/Script` + `Templates/arpg-ts-chinese/Script`）把审计出的不一致一次修完，其中多条属**静默改语义 / 让游戏起不来**级别：
    - **属性键**：两套口径写清（数据文件用 id，脚本里读运行时角色用属性名）；锁血/秒杀改成「先按属性名、再按 id、最后硬编码名」三层查找（旧实现先按 id，永远落空）。
    - **事件指令**：`setNumber` 改 `operation`+`operands` 数组、`setString` 改 `operation`+`operand`（旧实现会把对象 `String()` 成 `[object Object]`、值归 0）；新增引擎中文名别名表（「条件分支」旧被当 `if` → 条件被清空恒真；「设置文本」旧被当 `setString` → 静默无效）；`callEvent` 保留 `eventArgs`/`eventResult`。
    - **数据表**：属性表/枚举表按 `keys`/`strings` 树查找与追加（旧实现当 id 字典 → 顶层多垃圾键、原条目没改还返回 ok）；新建变量补 `sort`/`note`（缺 `sort` 时运行时 `groups[item.sort].push` 抛错 → 游戏起不来）。
    - **存档**：槽位只认 `saveNN.save`（本机 `Save/` 里 `save00.json` 19 字节垃圾文件曾被当存档并整份重写）；不再写 `hp`/`mp`/`level` 死字段。
    - **写盘**：数值数组回写时重编码 `code`/`terrains`（旧路径「报成功但地图没改」）；`writeAtomic` 沿用原文件行尾与尾换行（引擎是 CRLF + 无尾换行，旧实现每次写盘整文件 diff）。
    - **其它**：秒杀只打敌对阵营（`Team.relationMap`）；`rle.verify*` 兜住异常；插件 lint 认 `export default <标识符>`。
    - **验证**：真机浏览器量色与对拍脚本 + 临时工程副本上跑 MCP 全链路（数组回写、树表写入、路径门禁、体检不崩）+ 36/36 套通过 + 51 项锚点全绿 + 部署 MD5 逐项一致。
    - 同批（v1.10.10）：主页「AI 助手」卡片由绿改红（图标 + 进入徽章，红 `#ff5252`、徽章文字对比度 4.78:1 达 WCAG AA），静态健康新增「必须红、不许绿」+「样式源与构建产物同时落地」两条断言。

62. **输入框打 @ 直接选工程文件（v1.10.12，2026-09-24，落地路线图 G-3 / 第 16 轮）**：
    - **动机**：用户说「这个技能 / 这个角色」时，模型只能靠检索去猜，猜错就等于改错文件。
    - **做法**：宿主新增 `POST /files`（`list_scripts` + `list_resources` 合成去重，**排除图片与音频** —— 真工程 4849 个文件里 4267 个是图片音频，按工程缓存 60 秒，`refresh:true` 强制重扫，进页面即预热）；面板打 `@` 弹候选浮层（`↑↓` 移动 / `Enter`·`Tab` 选中 / `Esc` 只关浮层、不顺手停掉正在跑的一轮），文件名命中优先于目录命中、同分路径短在前，插入把 `@查询词` 整段换成 `@完整路径 + 空格`（前后的字都不动）；系统提示词补一句「`@相对路径` 是用户点选的文件，直接按它办事；没点选时不要自己造 `@路径`」。
    - **口径**：`@路径` **只是文本** —— 不进任何协议、也不改工具入参，点选这个动作不会变成第二条写盘通道。
    - **验证**：`test-ai-session.cjs` §13（真宿主 + 真工程 561 条候选，7 条断言）；`test-static-health.cjs` 把 5 个纯函数抽出来在 vm 里**实跑**（邮箱 `a@b` 不误触发、跨行失效、文件名优先排序、插入不吃字、光标落点）。全量 36/36 套通过。
    - **未做**：真机 GUI 走查（面板 DOM 属浏览器代码，本项目一贯只做静态契约 + vm 实跑断言）。

63. **文档 SSOT 自身的版本号漏洞（2026-09-24，本轮）**：
    - **现象**：`HANDOFF.md:22`（第一层 1.1「客观事实」表格）写着 `当前版本 v1.5.3`，与同文件头部的 `v1.10.12` 自相矛盾 —— 第一层本该是最准的一层。
    - **根因**：`build.cjs` 的级联同步正则只认「当前版本：」写法，而 1.1 表格那行分隔符是 `|`（`| 当前版本 | \`v1.5.3\` |`），**从 v1.5.3 起再没被同步过**（注释里还写着"一次覆盖全部出现处"，其实没覆盖）。
    - **改法**：分隔符从「只认 `：`」放宽到「`：` 或 `|`」，两种写法一起覆盖；`test-static-health.cjs` 新增断言把「HANDOFF 3 处版本号必须都等于 `manifest.json`」钉住（已用变异测试验证非空转：故意把 1.1 那行改回 v1.5.3 → 断言当场红）。
    - **顺带更正**：MCP 工具数 36 → **41 项（模型可见 40 项）**（v1.10.7 新增 `list_attributes` / `read_tilemap` 后未同步）；HANDOFF 头部与 3.2 的「最近更新」日期 09-13 → 09-24。

64. **@ 引用的四轮修补 + 对话面板补齐（2026-09-24，用户实测反馈「@ 功能有问题」「可以了但不够好用」）**：
    - **① 打 @ 永远说「读不到工程文件清单」**（用户报）：`/files` 的预热跑在「用户刚点进 AI 助手页」那一刻，而宿主（node 进程）正是那一刻才被拉起 —— 第一发必然失败；失败后 `mention.failed` **永久锁死**（`if (!mention.files && !mention.loading && !mention.failed)` 才重试），此后整天打 @ 都是"读不到"，而宿主其实早就起来了（实测 curl `/files` 返回 561 条）。修法：预热前 `await ensureHost()`（幂等：活着直接返回，没起来才拉进程）+ 失败改为 **3 秒节流重试**且成功后复位。**当时的提示话术把锅甩给宿主（"AI 宿主没起来？"），宿主是无辜的。**
    - **② @ 前面必须是空白字符**（中文写作几乎用不上，用户说"不够好用"的主因）：老判据 `if (at > 0 && !/\s/.test(text[at-1])) return null` 是英文习惯（空格分词），中文不会先敲空格再打 @ ——「帮我把@启动游戏事件 改成…」**根本不弹浮层**。放宽为「前一个字符是字母或数字才算邮箱/路径片段误触发」，`test@example.com` 照旧不弹。
    - **③ 候选中看不出为什么匹配**（用户勾选的方向）：命中片段高亮（文件名列与右侧路径列都高亮；逐段 `escapeHtml` 拼装，不在转义后的串上找下标，文件名带尖括号也不会破坏浮层 DOM）。
    - **④ 首屏候选不对路**：`mentionMatches('')` 原本 `files.slice(0,40)` = 宿主拼串顺序，**真工程实测前 40 条全是 `.ts` 脚本**；改成只按类型排序后又走到另一个极端 —— 单一类型占满（事件本身就有 40+ 条，首屏照样看不到别的类）。最终改为**按类型轮转**（事件一条、场景一条、技能一条…）：实测首屏 12 条覆盖 12 类，前 40 条 14 类各 3 条。
    - **⑤ 对话面板补两颗按钮**：回答上的**「复制」**（复制用户看到的正文，按类名剪掉按钮自身的字）与**「重新生成」**（只挂最后一条回答 —— 每条都挂会分不清点哪颗、旧的那颗一点就把后面全截掉；复用 `/session/rewind` 的重发链路，不另造截断路径；**必须在 `endTurn()` 之前挂**，它靠 `currentTurn` 找正文槽）。样式补 `.yami-ai-message.assistant:hover` —— 原来只写了 `.user:hover`，回答里的按钮会**永远隐身**。
...
66. **真机走查（MCP 驱动）抓到「重发 / 重新生成」轮次号错位 —— 会丢对话（v1.11.3，2026-09-24）**：
    - **现象**：走查第 5 条点「重新生成」，它重发的不是刚发的那条，而是会话里更早一条，并**把中间新增的对话一起截断了**。
    - **根因两层**：① 面板的 `state.userTurnSeq` 只在**回放历史**时被推进；重开面板时屏幕是空白的（不回放），计数从 0 起，而宿主那边已有 N 条用户消息 → **整体错一位**，`rewindTo` 截到了错误的轮次。② **第一版修法（在 `refreshContext` 里对齐）没生效** —— 对齐发生在 `attachRegenerate` **之后**，而按钮的闭包在挂上那一刻就把号钉死了。
    - **修法**：抽出 `alignTurnSeq()`（`ensureHost` → 问 `/status` 的 `userTurns` → `Math.max` 只增不减），在 `runMessage` **落用户气泡之前**调用；宿主 `/status` 新增 `userTurns`（与回放、rewind 同一口径：压缩检查点不计）。
    - **影响面**：用户消息上的「重发」「编辑」用的是同一套号 —— 即"有历史会话 + 重开面板"之后，任何一次「重发」都会截到错误位置。这正是第 12 轮 G-1 验收没照到的（那套测试用干净会话，两边序号恰好都是 0）。
    - **验证**：真机前后对比 —— 修前点「重新生成」后用户消息只剩 `["我要抄远征工会的代码…"]`；修后 `["我要抄远征工会的代码…", "只回答一个字：行"]`（历史保住、重发的是刚发那条）。静态健康加 3 条断言。
    - **同轮走查其余结论**：打 `@` 各类候选 ✓；`@启动` 高亮 ✓；回答下「复制 / 重新生成」都在 ✓；点「复制」toast 报字数且**剪贴板里确实是正文**（不含按钮文字）✓；审批续跑顺序未做（要真改工程文件）；在场感知口径做不到（要画布操作）。
    - **两个诊断陷阱（记下来免得重蹈）**：**a.** 一度以为"消息没进会话" —— 其实是**自己点的「重新生成」把它截断删了**，而查文件在截断之后（时序骗人）。**b.** 一度以为"编辑器被判成试玩环境" —— 页面里有两套 session key，`-playtest` 那个是**以前在试玩窗口留下的旧值**，与当前用哪个 key 无关（判定规则是 URL 是否含 `/app/dist/`，一直是对的）。

67. **真机走查收尾：在场感知「停在」把容器当控件名（v1.11.4，2026-09-24）**：
    - **⑥ 审批续跑顺序 ✓ 通过**：审批卡 2 秒内弹出（带差异预览 + 两档授权 + 取消/执行）；勾「本文件不再确认」+ 点「执行修改」后 —— 卡**即时收起**、notice 报「已记住：新建脚本 · …」、文件**真的落盘**、且续跑**以自己的回合组承接**（回合组计数 2→3，正文继续增长、状态推进「继续处理（第 N 步）」）。这正是第 52 项那套修法要的效果。
    - **⑦ 在场感知口径 ✓ 通过**：鼠标压在大容器上时，「选中「xxx」」被正确保留（没有被"停在"顶掉）。
    - **⑦ 顺带抓到并修掉一个 bug**：`describePresenceElement()` 的兜底分支把**容器**当成了控件 —— 鼠标停在资源树空白处，顶栏报出「停在「Assets! 事件插件场景技能角色粒子物品音频装备状态Du」=ngeon AssetsUI」，把整棵树 13 个文件夹的名字拼成了"控件名"（约 40 字，**正好从 60 字上限底下溜过去**）。修法：兜底只接受**可见文字子元素 ≤ 2** 的元素，容器直接返回 null。
    - **验证**：真机前后对比 —— 修前报整棵树；修后 `选中「循环」·文件「插件」选中中` ✓。静态健康加一条断言钉住判据。
    - **自我纠正一条**：⑥ 一开始以为"auto 模式写盘没审批"是 bug —— 查下来是 **AI 自己用了 `dryRun` 预览**（它那句"模板生成好了，你要的内容…"就是在征求确认），`isPreviewOnly` 让 `needsApproval` 为 false，**设计如此**；明确要求"直接创建、不要预览"后审批卡正常弹出。**判据**：光看工具卡片上的 `+N 行` 不够（dryRun 也会返回差异），要看**文件是否真落盘**。
...
| 界面细节验收（思考块 / 过程区 / 成本行 / 打断手感） | **已由助手真机收口（2026-09-24，MCP 连 9222）** | 真跑一轮读工程的提问后逐项核对：思考块段头「已思考 1 秒 · 125 字」✓；过程区摘要「执行过程 1 步 · 列出脚本」✓（`data-steps` 内部计数器是 0，但**界面文案正确**，不是缺陷）；成本行「本轮 2 次调用 · 23.6k tokens · 约 0.0140 元」+「余额」✓（本轮增量口径对）；打断手感 ✓ —— 点「停止」后状态变「已打断」、按钮由「停止」复位成「发送」、**已输出内容保留不清空** |
...
| 对话导出的界面手感（工具条【导出】/ 历史【导出】【导出全部】） | **已由助手真机收口（2026-09-24）** | 点工具条【导出】→ toast 报出文件名「已导出这段对话：工程里一共有多少个脚本？用一句话回答-2026-09-24.md」；落盘 `%APPDATA%/DanJuanDevSuite/exports/`（3033 字节，**没污染工程目录**）；排版 ✓：标题取首条用户消息、元信息（导出时间/会话 ID/规模「2 轮对话 · 14 条消息（另有 7 条工具结果未收进正文）」）、正文按「你 / AI 助手」分节，且**如实标注未收进正文的条数**，不假装完整 |
| 进面板是否回放上次那段对话（G-12） | 待用户确认 | 面板侧行为，静态契约已钉（`restoreLastSession` / `messagesPristine`）；真机由用户进页面看：屏上应当出现上次那段对话 + 一句"这是上次那段对话" |
| 编辑器热更新失败时的真机表现（F3） | **已归档，不再跟进（2026-09-24 用户明确「可以无视了」）** | 失败会挂到工具结果上让模型转告用户；"编辑器内存是否真的刷新了"只能由用户看（引擎未暴露 `YamiEngine` 时热更新本就不可用）。要制造这个条件才复现得了，成本高收益低；正常热更新路径已被整包快照测试覆盖 |
...
| 测试偶发（2026-09-15 记录，2026-09-24 复查） | 未复现，已补留档机制 | **2026-09-15**：全量跑 6 次有 1 次 `test-mcp-ai-tools` / `test-playtest-smoke` / `test-acceptance` 退出码非 0（单独跑与紧接着的重跑都全绿，实际 0 FAIL），负载/时序敏感，未抓到是哪条断言。**2026-09-24 复查：连跑 6 次全绿（36/36），未复现** —— 这不等于已修，只说明当前负载下不触发（疑似与"夹具整份拷贝真工程"有关，第 53/54 项把夹具从 440.6MB 瘦到 18MB 后负载大降）。已把当年那句"下次遇到先留日志"落地：`tests/run-all.cjs` 现在**任何套件失败就当场重跑一次、把两次输出写进 `_last-run.txt`**（终端打印路径），下次复现时可直接对比首跑与重跑输出判断是不是时序 |
...
| @ 引用工程文件的真机表现（第 16 轮） | 待用户确认 | 宿主侧已由 `test-ai-session.cjs` 第 13 节真链路验收（561 条候选 / 排除图片音频 / 缓存与重扫）；面板 DOM 渲染属浏览器代码，本项目一贯只做静态契约 + vm 实跑断言。重启编辑器后请试：打 `@` 弹候选 → 打 `@启动` 第一条应是「启动游戏事件」→ 回车插入后输入框出现 `@Assets/! 事件/@1 启动游戏事件.<GUID>.event ` → `Esc` 只关浮层（不会停掉正在跑的一轮） |
| 审批续跑的顺序修复（【52】） | **已收口（2026-09-24 结清）** | 宿主侧由 `test-ai-repair.cjs` §6b 真链路验收（事件流上有 start / 工具 start+done / 逐字正文 / 最终结果）；面板 DOM 渲染属浏览器代码，本项目一贯只做静态契约 + vm 实跑断言 |
| **@ 引用工程文件 · 真机复验（第 16 轮）** | **已实测通过（2026-09-24）** | 助手经 CDP 连编辑器实测：点主页卡片能开面板（v1.11.1 修复成立）；打 `@` 弹出 40 条候选且**首屏按类型轮转**（事件/场景/界面/角色/技能/道具各一条）、提示行报「候选来自工程 561 个文件」、浮层实测 264px 高可滚动；打 `@启动` 收敛到 1 条；回车插入得 `@Assets/! 事件/@1 启动游戏事件.896108c7557627ff.event `、光标停在 46（引用末尾）、浮层自动关闭且**没有误发消息** |
| **playtest_smoke · 真机链路** | **已实测通过（2026-09-24）** | 真机跑通：起试玩 → `playtest_smoke` 真下发 Enter（`executed: [{key:'Enter', ok:true}]`）、前后诊断对比（drawCalls 8→158）、verdict `ok`；`get_runtime_state` 报 fps 180 / 场景 actors 5 / elements 96 / textures 71。**降级路径也验了**：试玩没跑时它如实说「请先在编辑器里点【试玩】」，绝不假装跑过 |
| **试玩窗口的 `Scene.binding`** | **已实测确认真机有数据（2026-09-24）** | 试玩窗口里 `getSceneDetails()` 返回完整数据（actors 5 / visibleActors 5 / elements 96 / textures 71 / resolution 1920x1080 / camera），`getSceneEntities()` 给出场景 id、路径与逐个角色的 presetId/坐标/碰撞/导航/动画 —— 与 09-15 用户实测一致：运行时有这套数据，**这条不用改** |
| **在场感知口径 · 真机复验（第 49 项）** | **① 已通过；② 抓到缺陷并修掉** | ① 高亮选中某项后鼠标移到画布，顶栏仍报「选中「循环」」而不是「停在「画布」」✓。② 鼠标停在文件列表的「全局插件」上时顶栏报出 **「停在「场景对象脚本界面元素脚本全局插件自定义指令」」**（把容器里 4 个文件名拼成一串）—— v1.11.4 只数了直接子元素，被一层 wrapper 挡住；已改为数整棵子树的可见文字叶子（见下一行） |
| **真机走查抓到并修复的 3 个缺陷（2026-09-24，助手实测）** | **已修 + 已加断言守卫（v1.11.6）** | ① **`/diagnose` 整条端点 500**：`buildDiagnosis` 的 `top()` 对 Map 调 `.slice`（updaterTotal / rendererTotal / eventTotal 本来就是 Map），只有真机调这个端点才走到 —— 后果是 AI 只能拿精简诊断，且这次崩溃被记进运行日志，用户看到「2 处异常」还以为是游戏报错（试玩窗口实测确凿：错误指纹直指 probe-core.js）。改用 `formatList`，输出形状不变。② **重启/刷新后进 AI 面板：屏上空白、上下文里还是上一轮 48 条消息** —— G-12 只修了「面板内重新激活」，冷启动那条路反而保持旧 sessionId；现在冷启动第一次进面板直接开新会话（旧对话都在【历史】里）。③ **顶栏「停在」把文件列表容器当控件名**（同上一行 ②）。三条都有断言守着（`test-static-health.cjs` 的「真机缺陷守卫」3 项）|
## 3.4 下一步建议

1. **真机 GUI 走查**：已用 CDP 连编辑器 9222 走掉大半 —— 打 `@` 候选与高亮 ✓、回答下的「复制」✓、「重新生成」✓（v1.11.3 那个会丢对话的轮次错位就是这条抓到的）、点主页卡片开面板 ✓、试玩冒烟 ✓、场景实体 ✓、在场感知口径 ✓（并抓出「容器当控件名」的缺陷）、审批续跑顺序 ✓（宿主侧 `test-ai-repair.cjs` §6b 真链路验收）。**只剩 1 条，只能由用户做**：
   **点试玩** —— 1.7 早就写明「画布拖拽、点试玩、手感与视觉判断仍只能由用户来做」：助手用 `editor_action playtest` 反复启停会把编辑器的「试玩中」标志与真实窗口搞到不一致（2026-09-24 实测：reload 过试玩窗口之后再点，窗口不再出现）。**这条别再让助手代劳。**
   走查前先 `node build.cjs --deploy`；插件代码改动后**刷页面即可加载**，不必重启编辑器。**注意**：真机走查要开着编辑器，而跑全量自测前必须关掉它（见 1.7 那条警告），两者别同时做。
2. 用一个真实需求（例如改主菜单某处 UI）走完整链路：搜索 → 精读（`key`）→ 预览 → 确认 → 编译 → 变更小结 → 撤销，检验端到端手感。
3. 3.3 里那 13 条的收口状态（2026-09-24）：**助手能收口的已全部收完** —— 9 条已由助手真机收口（打 `@` 候选与高亮、回答下的「复制」「重新生成」、点主页卡片开面板、试玩冒烟、场景实体、在场感知口径、界面细节四项、对话导出、审批续跑顺序），**3 个真机缺陷当场修掉**（v1.11.6），**最后 1 条**（编辑器热更新失败时的真机表现）经用户 2026-09-24 明确「可以无视了」→ **按已知限制归档，不再跟进**（正常热更新路径已被整包快照测试覆盖）。**注意**：表里仍标「待用户确认」的行是**留给用户眼睛看的**（不是没做完），其中 `playtest_smoke`／`Scene.binding`／`@ 引用工程文件` 三条已被下方同名「已实测通过」行收口，仅状态格未同步。**收完再谈加新能力** —— 没有验收的功能等于没做。
4. 视使用情况决定是否补 P2 项（子代理委派）；「编辑重发」第 12 轮、「对话导出」第 13 轮、「@ 引用工程文件」第 16 轮、「复制回答 / 重新生成」第 17 轮均已落地。
