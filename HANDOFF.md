...
3. 3.3 里那 13 条「待用户确认」：**5 条已由助手真机收口（2026-09-24）**，另有 3 个真机缺陷当场修掉（v1.11.6）。**还剩 4 条要真跑一次模型对话**（消耗 API 额度；其中「审批续跑」还要真改一个工程文件，需用户点头）：界面细节（思考块 / 过程区 / 成本行 / 打断手感）、对话导出的界面手感、编辑器热更新失败时的表现、审批续跑顺序。**收完再谈加新能力** —— 没有验收的功能等于没做。
