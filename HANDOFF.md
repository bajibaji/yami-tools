...
- **现象**：确认卡上点「执行修改」**毫无反应**，再点还是没反应 —— 用户只能重启编辑器。三层原因叠在一起：
  ...
  2. `/approve` 是普通 POST、**不是 SSE**：确认之后整轮在后台跑完（可能还有好几轮模型调用 + 工具）才一次性返回，期间界面上一个字都不会动 —— 用户看到的就是"卡住了"；
...
- **铁律**：
  ...
  3. 长耗时又**没有流式输出**的操作，必须显式告诉用户"它在干活"+ 计时，并且保证随时能停；
  4. 打断路径必须**自己负责**把界面状态解开，不许赌"那个异步早晚会回来"。
- **验证**：`ai-agent.js` 的 `decide()`（`!state.pending` 才拦，`state.deciding` 防连点，执行中计时 + 提示）与 `stopStream()`（兜底清 busy 与挂起卡）。
