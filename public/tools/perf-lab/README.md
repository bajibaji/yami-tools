...
工具页左上角有「**使用说明？**」按钮，第一次使用先点它；下面的小节是同样的完整步骤。
...
**不要在录制 DevTools Performance 时运行真机逐帧探针。** 两者都会使用 `requestAnimationFrame` 和函数包装，混在同一次录制里会让 CPU Profile 采到探针自身的 `tick/hookGame`，热点结论失真。正确做法是同一场景分别复现两次：一次只录 DevTools，一次只运行探针。
...
1. 在分析台侧栏点「复制探针脚本」；
2. **关闭并重新打开游戏窗口**，确保旧探针包装已卸载；然后在 Electron 游戏 DevTools **控制台**粘贴并回车（不修改工程、不写文件）；
3. 正常游玩目标片段（卡顿场景）；
4. 在控制台执行：
   ```js
...
- v2 探针按 Yami 全局身份把模块命名为 `Scene / EventManager / UI / Camera / AudioManager` 等，并记录每个超帧的归因覆盖率与未归因时间；
- 分析台「超帧定位」页会汇总：哪个模块/事件**最常导致超帧**、累计多少毫秒，并列出最差帧 Top 60。

只有归因覆盖率足够、且某项单帧耗时确实显著时，分析台才会标成“高置信元凶”。证据不足时会明确显示“无法点名”，不会拿几百微秒的事件冒充根因。
...
- **超帧定位**：探针数据下的**采集期间总耗时排行**（更新器/渲染器/事件）、超帧元凶（只显示累计 ≥0.05ms 的有效项）与最差帧列表。
