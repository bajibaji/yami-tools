    ...
    dock.innerHTML = `
      ...
      <div class="yami-perf-dock-footer">
        <div style="color: #808080; display: flex; align-items: center; gap: 8px;">
          <span id="yami-version-badge" style="color: #0080c0; cursor: pointer; text-decoration: underline;" title="点击检查 GitHub 最新版本">v0.7.1 (检查更新)</span>
        </div>
...
// (escapeHtml 已统一提升为模块顶级公共转义函数)
    ...
    function refreshVersionBadge() {
      ...
      const probe = window.__YAMI_PERF_PROBE__;
      const cur = (probe && probe.version) ? probe.version : '0.7.1';
    ...
    if (versionBadge) {
      versionBadge.addEventListener('click', async function(e) {
        ...
        if (res.hasUpdate) {
          ...
          showToast('当前已是最新版本 (v' + (probe.version || '0.7.1') + ')');
          refreshVersionBadge();
        }
      });
    }
